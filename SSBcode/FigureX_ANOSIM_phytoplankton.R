...
#####
#ANOSIM method A
  #ALL years, R = 0.7693, pvalue = 1e-04
  group.df <- SSB %>%
    select(year, sample_date) %>%
    distinct(year, sample_date, .keep_all = TRUE)
  #pivoting table to match abundance matrix 
  p.pivoted <- SSB %>%
    select(sample_date, taxa_id, taxa_count) %>%
    group_by(sample_date, taxa_id) %>%
    summarise(taxa_count = sum(taxa_count))  %>%
    pivot_wider(names_from = taxa_id, values_from = taxa_count, values_fill = 0) %>%
    ungroup() %>% select(-sample_date)
  #converting p.pivoted df into matrix
  p.matrix = as.matrix(p.pivoted)
  #ANOSIM
  ...
  ano
#ANOSIM method B 
  #ALL years, R = 0.7693, pvalue = 0.001
  p.dist <- vegdist(p.pivoted)
  p.ano <- with(group.df, anosim(p.dist, year))
  summary(p.ano)
  plot(p.ano)
#                          Call:
#                            anosim(x = p.dist, grouping = year) 
#                         Dissimilarity: bray 
#                          
#                          ANOSIM statistic R: 0.7693 
#                          Significance: 0.001 
#                          
#                          Permutation: free
...
#                            90%   95% 97.5%   99% 
#                            0.216 0.304 0.404 0.508 
#                          
#                          Dissimilarity ranks between and within classes:
#                                 0%     25%    50%    75%  100%  N
#                         Between  1    30.00   43.0  54.5   66   47
#                          2019    3    8.00    13.0  15.5   18   3
#                          2020   4      5.75    13.5  20.5   23   6
#                          2021   2     11.25    15.5  27.5   32   10
...
#####
  #ANOSIM method A
  #Refernce and Manipulation 2, R = 1 (?), pvalue = 0.017
  SSB2 <- SSB %>%
    select(sample_date, taxa_id, taxa_count) %>%
    filter(sample_date < '2020-01-01' | sample_date > '2020-12-31') %>%
    group_by(sample_date, taxa_id) %>%
...
#####
  #ANOSIM method A
  #Refernce and Manipulation 2, R = 0.9937, pvalue = 0.0084
  ...
  #ANOSIM method B 
  #ALL years, R = 0.9937, pvalue = 0.01
  p.dist3 <- vegdist(SSB3)
  p.ano3 <- with(group.df3, anosim(p.dist3, year))
  summary(p.ano3)
  plot(p.ano3)
...
#####
  #ANOSIM method A
  #Refernce and Manipulation 2, R = 0.3333, pvalue = 0.1429
  SSB4 <- SSB %>%
    select(sample_date, taxa_id, taxa_count) %>%
    filter(sample_date < '2021-01-01') %>%
    group_by(sample_date, taxa_id) %>%
  ...
  #ANOSIM method B 
  #ALL years, R = 0.3333, pvalue = 0.135
  p.dist4 <- vegdist(SSB4)
  p.ano4 <- with(group.df4, anosim(p.dist4, year))
  summary(p.ano4)
  plot(p.ano4)
